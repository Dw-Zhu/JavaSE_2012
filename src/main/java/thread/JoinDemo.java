 ...
 * 线程提供了一个方法:
 * void join()
 * 该方法允许一个线程在另外一个线程上等待,知道该线程结束后才会结束等待进行自己后续操作.
 * 例如:
 * A,B是两条线程
 * 当A线程调用了B.join()后,A线程处于阻塞状态,直到B线程结束,A线程才能接触阻塞执行join
 * 方法后续代码.
 * join方法可以协调线程的同步运行.
 * 同步运行:多个线程执行存在先后顺序执行.
...
public class JoinDemo {
    ...
    public static void main(String[] args) {
        /*
        一个方法的局部内部类中若引用了这个方法的其他局部变量是,这个变量必须被声明
        为final的(JDK8之后,可以不再显示的使用final关键字修饰这个局部变量,但是
        如果被该方法的其他局部变量引用时,它仍然是final的)
         */
//        final boolean isFinish=false;
        ...
        Thread show=new Thread(){
            public void run(){
                ...
                //阻塞show线程,知道download执行完毕后再进行后续工作
                try {
                ...
                if(!isFinish){//如果图片没有下载完毕
                    //当一个异常抛出到线程任务的run方法之外是,这个线程就会结束.
